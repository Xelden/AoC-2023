...
ull solve(std::ifstream &file)
{
    ...
    while (!reached_end) {
        ...
        for (auto [a, b] : get_next_possible_moves(curr_char)) {
            ...
            if (i > 2 && curr_char == 'S') {
                reached_end = true;
                corners.push_back(std::make_pair(x + b + 1, y + a + 1));
                break;
            }
            ...
            for (auto [c, d] : get_next_possible_moves(curr_char)) {
                ...
                // Save corners for shoelace
                if (curr_char == 'L' || curr_char == 'F' || curr_char == 'J' || curr_char == '7') {
                    int tmp = curr_pos.first;
                    curr_pos.first = curr_pos.second + 1;
                    curr_pos.second = tmp + 1;
                    corners.push_back(curr_pos);
                }
    ...
    // Shoelace formula
    ll total = 0;
    for (auto it = corners.crbegin() + 1; it != corners.crend(); ++it)
        total += (it - 1)->first * it->second - it->first * (it - 1)->second;
    total += corners.front().first * corners.back().second - corners.back().first * corners.front().second;

    return (std::abs(total) - i) / 2 + 1;
}
